...
<div id='body_top'>
	<div id='body_bottom'>
		<div id='wrapper'>
			<div id='top_panel' >
				<div id='panel'style="z-index:99999999999999999999999999999999999999999999999999999999999999">
					<div id='server'>
...
<div id="guild_top"></div>
 ...
 <table class='clear' border='0' align="center">
            ...
            <?php //ostavqm ti mqsto za izqva :D
             $select_guild = mssql_query("Select TOP 5 * from [Guild] order by [G_Score] desc");
                  for($i=1; $i<mssql_num_rows($select_guild)+1;$i++){
					...
					$total_players = mssql_num_rows(mssql_query("Select * from GuildMember where G_Name='".$top_guild['G_Name']."'"));
					 // qvno sum go napravil posledno po pe4enegursko bez da kopira ama sum zabravil :D
					 echo 
					 "	
					    <tr title='".GuildMark(bin2hex($top_guild['G_Mark']), 30)." + Name + tuka slaga6 izqva :D'>
					      <td>{$i}</td>
						  <td>{$top_guild['G_Name']}</td>				
						  <td>{$total_players}</td>
						  <td>{$top_guild['G_Score']}</td>
						  <td>".guild_res($top_guild['G_Name'])."</td
					    </tr>
					 "; 
				    }
			 ?>	
