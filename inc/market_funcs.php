...
require $_SERVER['DOCUMENT_ROOT']."/configs/config.php";
		if($option['item_hex_lenght'] == 64){
			$chk_serial = hexdec(substr($item,32,8));
		}
		else{
...
function all_items($account,$table = "warehouse",$user_col="AccountId",$item_col="items"){
	  ...
	  if(season())
		{
		   $season = season();
    	   if(phpversion() >= "5.3"){
		     	$itemsa = mssql_fetch_array(mssql_query("SELECT CONVERT(VarChar({$season[1]}),Items,2) as Items  FROM [".$table."] WHERE [".$user_col."]='".$account."'"));
		     	return strtoupper(($itemsa['Items']));
		   }
		     else{ //5.2.9
		   	   return substr(mssql_get_last_message(mssql_query("declare @items varbinary(".$season[1]."); set @items=(select [".$item_col."] from [".$table."] where [".$user_col."]='".$account."'); print @items;")),2);	 
		   }
		}
        else{
        	return false;
 ...
 function market_buy($item_id,$buyer){
	 require $_SERVER['DOCUMENT_ROOT']."/configs/config.php";
	  ...
	  $mycuritems  = all_items($user);
	  $item        = ItemInfoUser($market_pr['item']);
	  $slot        = smartsearch($mycuritems, $item['x'], $item['y']);
         
      $test        = $slot * $season[0];
 
      $mynewitems  = substr_replace($mycuritems, $market_pr['item'], $test, $season[0]);    
...
$settings    = mssql_fetch_array(mssql_query("Select * from [DTweb_Market_Settings]"));
$rez         = mssql_fetch_array(mssql_query("Select [AccountId],[Money] from [warehouse] where [AccountID]='".$user."'"));
...
for($i=0, $max = mssql_num_rows($all_banned);$i < $max; $i++){
	...
	*/
	...
	if (preg_match("/{$item_serial}/", $mycuritems)) {
		  ...
		  if($settings[4] != 0){
	  	     $update_market   = mssql_query("INSERT INTO [DTweb_Market] ([item_type],[item_id],[item],[zen],[soul],[bless],[chaos],[creation],[life],[stone],[credit],[rena], [seller],[start_date],[is_sold],[seller_ip],[end_date],[name],[level],[skill],[luck],[options],[excellent]) VALUES('".$item["type"]."','".$item["id"] . "','".$item_serial."','".round($upd_zen)."','".$soul."','".$bless."','".$chaos ."','".$creation."','".$life ."','".$stone ."','".round($upd_credit) ."','".$rena ."','".$user."','".time()."','0','".ip()."','".$end_date."','".($itemtem['name'])."','".$itemtem['level']."','".$itemtem['srch_skill']."','".$itemtem['srch_luck']."','".$itemtem['opt']."','".$itemtem['exl']."')");
			 }
		  else {
		     $update_market   = mssql_query("INSERT INTO [DTweb_Market] ([item_type],[item_id],[item],[zen],[soul],[bless],[chaos],[creation],[life],[stone],[credit],[rena], [seller],[start_date],[is_sold],[seller_ip],[name],[level],[skill],[luck],[options],[excellent]) VALUES('".$item["type"]."','".$item["id"] . "','".$item_serial."','".round($upd_zen)."','".$soul."','".$bless."','".$chaos ."','".$creation."','".$life ."','".$stone ."','".round($upd_credit) ."','".$rena ."','".$user."','".time()."','0','".ip()."','".($itemtem['name'])."','".$itemtem['level']."','".$itemtem['srch_skill']."','".$itemtem['srch_luck']."','".$itemtem['opt']."','".$itemtem['exl']."')");	
...
$itemsa      = array($mycuritems);
	if (strlen($item_serial) != $season[0]) {
	    $messages[] = phrase_market_invalid_item;
	}
