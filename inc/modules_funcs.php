...
function guild_res($guild){ //последно как бях го правил първоначално го бях направил да копира това от характер в гилд мембърс после някой нещо искаше и му преправях и ги копира6е в гилд ама тука е въсвем различно :Д ще го видя 
	$a = array();
...
//ne6to ne moga da namerq funckiqta za admina 4akai da vidq pti men da ne se okaje 4e tozi ne6to e pravil i emahal vuzmojno e ok
function balls($var){
	$pic ='';
	$get_config = simplexml_load_file('./configs/mod_settings/r0toto_settings.xml');
...
function do_reset_stats()
	{
		...
		else
		{
			...
			$cur_money = mssql_fetch_array(mssql_query("Select * from DTweb_Bank where memb___id = '". $acc ."'"));
			$cur_cr    = mssql_fetch_array(mssql_query("Select * from Memb_Credits where memb___id = '". $acc ."'"));
			$new_money = ($cur_money['Zen'] - $option['rs_zen']);
			$new_cr    = ($cur_cr['credits'] - $option['rs_cr']);
			...
			elseif($new_money < 0 || $new_cr < 0){
				$show_msg['error'][] = "Not enough resources";
			}
			elseif($is_online === 1)
			{
			...
			{
				 mssql_query("Update Memb_Credits set credits = credits-'".$option['rs_cr']."' where memb___id='".$acc."'");
				 mssql_query("UPDATE Character SET Strength = 0, Dexterity = 0, Vitality = 0, Energy = 0,  LevelUpPoint = ".$new_lvlupp." WHERE Name='".$character['Name']."' AND AccountID='".$acc."'");
				 mssql_query("Update DTweb_Bank set Zen = Zen -'".$option['rs_zen']."' where memb___id='".$acc."'");
...
function send_gifts($char,$receiver,$sender){
    ...
    $chk_usr_zen  = mssql_fetch_array(mssql_query("Select * from [DTweb_Bank] where [memb___id] = '{$sender}'"));
