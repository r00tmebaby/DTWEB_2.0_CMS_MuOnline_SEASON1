...
$list_query = "SELECT TOP {$lines} * FROM [DTweb_Market] where id Not in (Select TOP {$limit} [id] FROM [DTweb_Market] ".$q_change." order by [start_date] desc) ".$q_change1." order by [start_date] desc";
$all_market = mssql_num_rows( mssql_query("SELECT * FROM [DTweb_Market] ".$q_change.""));
