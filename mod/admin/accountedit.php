...
////////////////////////  
    ...
    if(isset($_GET['account'])){
            ...
            $acc_chars     = mssql_query("Select Name from Character where [AccountID] = '".$account."'"); 
...
if($checkitem['count'] == 0){
	...
	mssql_query("Insert into [DTweb_GM_Modules_Logs] ([module], [date], [gm_name], [ip],[data] ) values ('ItemAdder', '".time()."', '".$logged."', '".ip2long(ip())."','".$data_enc."')");
	  refresh();
		...
		if($gm_level > 2){
	$td2 = "   <tr><td class='title'>Last game login</td><td align=left>".$memb_info['out__days']."</td></tr>
