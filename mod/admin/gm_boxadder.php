...
if ((isset($_POST['add_box'])) && (isset($_POST['box'])) && (isset($_POST['count']))) {
	...
	if(!isset($_POST['clear_inv']))
	{
	    ...
	    elseif((smartsearch(all_items($_SESSION['dt_username']), 1, 1)) == 1337){
	    ...
	    else
		{
		 ...
		 for($k=0;$k<$requestedItems;$k++){					
					 $slot  = smartsearch(all_items($_SESSION['dt_username']), 1, 1);
					 if($slot <> 1337){
						$test  = $slot * 20; 
						$mynewitems  = substr_replace(all_items($_SESSION['dt_username']), $select[0], ($test), 20);  
	     ...
	     if($erris == 0){
			 ...
			 echo "<div class='success'>". phrase_box_adder_success ."<span class='bold'>{$requestedItems} x </span><img style='position:absolute;' src='imgs/items/special/box".$select[3].".gif' style='border:1px solid #000' width='26px'/><span style='margin-left:10px;'>&nbsp;". phrase_box_adder_success2 ."</span></div><br/>";
		 }
		 else{
			 ...
			 mssql_query("Update DTweb_GM_Box_Adder_Logs set {$column} = {$update_column} where end_date ='".gm_box_timer($gmuser)."'");
		     mssql_query("Update DTweb_GM_Box_Inventory set {$column} = {$update_bank} where memb___id ='".$gmuser."'");		   
		     echo "<div class='success'>". phrase_box_adder_success ."<span class='bold'>{$m} x </span><img style='position:absolute;' src='imgs/items/special/box".$select[3].".gif' style='border:1px solid #000' width='26px'/><span style='margin-left:10px;'>&nbsp;". phrase_box_adder_success2 ."</span></div><br/>";
	        }
		}
	       if (count($error) > 0)
	          {
...
<div style='width:500px' class='box_bank'>
	    ...
	    <div id='tab'text-shadow:1px 1px #fffs;background:#1A1715;padding:10px 10px; margin-bottom:5px;'>
		  ...
		  ". phrase_box_adder_info ."
		      <img src='imgs/items/special/box1.gif' width='26px'/> x <span style='color:#FDFDFF;'>".$bank1."</span> 
              <img src='imgs/items/special/box2.gif' width='26px'/> x <span style='color:#FDFDFF;'>".$bank2."</span> 
              <img src='imgs/items/special/box3.gif' width='26px'/> x <span style='color:#FDFDFF;'>".$bank3."</span> 
              <img src='imgs/items/special/box4.gif' width='26px'/> x <span style='color:#FDFDFF;'>".$bank4."</span> 
              <img src='imgs/items/special/box5.gif' width='26px'/> x <span style='color:#FDFDFF;'>".$bank5."</span>
			  </br></br>
		   </div>
			  </br></br>
       <label style='color:#FF7E00'>". phrase_box_adder_info3 .box_counter($check_box['end_date'])."</label>
           <form class='form' style='width:500px' method='post'>
               <input class='form-control' type='number' min='1' placeholder='Max 120' max='120' name='count' size='3' maxlength='2' />
              ...
              <img src='imgs/items/special/box1.gif'/> x <span style='color:#93FF26'>".$box1."</span> / <span style='color:#EF430E;font-weight:600'>".$option['GM_'.$gm_info[1].'_box1']."</span>
              <img src='imgs/items/special/box2.gif'/> x <span style='color:#93FF26'>".$box2."</span> / <span style='color:#EF430E;font-weight:600'>".$option['GM_'.$gm_info[1].'_box2']."</span>
              <img src='imgs/items/special/box3.gif'/> x <span style='color:#93FF26'>".$box3."</span> / <span style='color:#EF430E;font-weight:600'>".$option['GM_'.$gm_info[1].'_box3']."</span>
              <img src='imgs/items/special/box4.gif'/> x <span style='color:#93FF26'>".$box4."</span> / <span style='color:#EF430E;font-weight:600'>".$option['GM_'.$gm_info[1].'_box4']."</span>
              <img src='imgs/items/special/box5.gif'/> x <span style='color:#93FF26'>".$box5."</span> / <span style='color:#EF430E;font-weight:600'>".$option['GM_'.$gm_info[1].'_box5']."</span>	
</center>";                                                         
  }
  else{
