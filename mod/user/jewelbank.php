			...
			{           	
		        foreach($warehouse as $item){
		        	if($item['hex'] == $settings[0]){				
		        		...
		        		$items         = all_items($_SESSION['dt_username']);				
		        		$i             = -1;
		        		$num           = 0;
		        		$broika        = 0;
		        		while($i < 119){	
		        			...
		        			if($i == $item_position && $num==0){
		        				$items= substr_replace($items,str_repeat('F',$season[0]),($i*$season[0]), $season[0]);
		        				$num++;
		        				$broika += $num;
		        			}
		        		}				
		   ...
		   $count = (int)$_POST["count"];   
	        ...
	        elseif($type <> $settings[0] || $jewels[''.$settings[2].''] <= $count || $jewels[''.$settings[2].''] <= 0 )
			{
	            ...
	            for($i = 0;$i < $_POST["count"]; $i++)
	            	{		
	            		$mycuritems = all_items($_SESSION['dt_username']);
						...
						{	            
	            			$mynewitems = substr_replace($mycuritems, $newitem, $test, $season[0]);	            			
	            			$update = mssql_query("
...
echo "		
	 ...
	 echo "</table> 
        ...
        $jewels_bas = mssql_query("Select * from [DTweb_Deposit_Settings] where [active]='1'");
	        ...
	        for( $i= 0 , $max = count($jewels_bas);$i < $max ; $i++ )
			{
               ...
               echo "<tr style='color:".$jewels_color.";'>
			            <td class='td'>".$jewels_name."</td>
			            <td class='tdinf'> ".$jewel_count."</td>
			         </tr>"; 	  
			  }
            }
    echo "</table>";
	...
	echo "Wrong Item Hex Configuration!";
}
