...
if (basename(__FILE__) == basename($_SERVER['PHP_SELF'])) {header("Location:../error.php");}else{
	...
	$database                     = '';           // Mu online database: default = MuOnline 
    $option['web_address']	      = "";      // IP(static IP/external) or Domain Name. Must use http:// !Important for PostBacks and API's
	$option['has_dl']             = 1;                     // (IMPORTANT) Dark Lord: 1 = enable, 0 =  disable
    $option['md5']                = 0;                     // MD5 1 =ON / 0 = OFF
    $option['debug']              = 1;                     // 1=Show Errors / 0 = Hidden Errors   	                                                 
	$option['default_admin']      = '';                // Default Admin Account
	$option['default_admin_ip']   = '';           // Default Admin Account
	$option['item_hex_lenght']    = 20;                    // S1 = 20 , S1-8 =32, S9-13 and IGCN = 64 / No other options are available, Do not fill anything else as the website wont work properly
	$option['max_sockets_show']   = 0;
	// туй не важи за тебе то е за нвите сезони
	//================================================================================================
	//Vote Panel
    $option['vote_credits']       = 30;              // How many credits for one vote
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //=================================================================================================	
    // Buy Credits
	// tova ne ti trqva sega nqma6 systema vps trqbva ti realno ip za da moje da vru6tat ot payment systemite
...
//PayPal Settings // ======================================================= 
	...
	$option['paypal_prices']   = array(
	    100   =>  1 ,   // Credits => Price  //Use 0.01 euro for testing 
		200   =>  2    ,   // Credits => Price
		500   =>  3    ,   // Credits => Price
...
//PaymentWall Settings
	...
	$option['rc_zen']             = 15000000;      // Zen required to reset a character
	$option['rc_zen_type']        = 1;             // Increase required zen for every reset: 1 = enable, 0 =  disable 
	$option['rc_stats_type']      = 1;             // Increase stats for every reset(1r=500,2r=1000,...): 1 = enable, 0 =  disable 
	$option['rc_max_resets']      = 999;           // Max resets
	$option['rc_stats_per_reset'] = 0;           // Bonus stats per reset
	$option['rc_clear_stats']     = 1;             // Clear stats after reset: 1 = enable, 0 =  disable	
	$option['rc_stats_for_sm']    = 50;           // Points for dw,sm and grm 
	$option['rc_stats_for_bk']    = 500;           // Points for dk,bk and  bm
	$option['rc_stats_for_me']    = 500;           // Points for elf,me and he
	$option['rc_stats_for_mg']    = 700;           // Points for mg and dum 
	$option['rc_stats_for_dl']    = 700;           // Points for dl and le
	$option['rc_bonus_points']    = 1;             // Bonus points 
	
	//================================================================================================	
    // Storage Items on Page
	$option['storage_pagi']         = 10; // Rows per page tova kolko da pokazva paginaciqta straniciraneto na edna stranica
    $option['storage_banned_items']	= array("24/11");   // Banned Items for Deposit 0/10 - type/id
// Example 0/1 = Kriss array("0/1","0/2"); = Kriss and katana ako ne se luja b6e 2 
	//================================================================================================
    // Add Stats                                       
	...
	//================================================================================================	                                               
    // PK Clear                                         
	$option['pkc_zen']            = 20000000;      // Zen required to clear pk level
	$option['pkc_zen_type']       = 1;             // Increase required zen for every pk level: 1 = enable, 0 =  disable 
	
	//================================================================================================                                                  
    // Reset Stats                                      
	$option['rs_level']           = 1;           // Level required to reset stats
	$option['rs_resets']          = 1;            // Resets required to reset stats
	$option['rs_zen']             = 50000000;      // Zen required to reset stats
	$option['rs_cr']              = 50;             // Credits request
	//================================================================================================
    // Grand Reset                                    
	$option['gr_level']           = 400;            // Level required to grand reset
	$option['gr_resets']          = 150;            // Resets required to grand reset
	$option['gr_zen']             = 100000000;      // Zen required to grand reset
	...
	$option['gr_credits']         = 1; 				// This is the reward name - rena stone item and etc
	//================================================================================================
	...
	//================================================================================================ 
    // Gift Module
	// za gift modula ima6e nqkuv report az misq 4e go fixnah no ne sum siguren trstvai
	$option['gift_res']           = array("rena","zen","credits","soul","bless","creation","life","chaos");
    $option['gift_procent']       = 0; // Add this percents to every send resource
	$option['gift_credits']       = 20;   // If this option is > 0, the cost is this value in credits, the multiplier wont work
    //================================================================================================   
	//Warnings Stack
	$option['warning_times']      = 3;   // How many warnings before the ban to be passed
//Tuka event countera se glasi po sledniq na4in zna4i imeto e purvoto wqsno
// posle ima6 data da kajem startova moje da e minalo vreme kakto v slu4aq no si pi6e6 4asa
// Repeat time e v secundi
// Open time e vremeto prez koeto mohgat da vlqzat predi da se zatvori portata :D
	...
	$option['Box_timer_GM2']     = 24 ;     // Box Timer for GM level 2 /in hours/
	
	/////////////////////////                         
	$option['GM_1_box1']         = 10 ;     // How many boxes +1 GM level 1 can add on day
	$option['GM_1_box2']         = 10  ;     // How many boxes +2 GM level 1 can add on day
	$option['GM_1_box3']         = 10  ;     // How many boxes +3 GM level 1 can add on day
	$option['GM_1_box4']         = 10  ;     // How many boxes +4 GM level 1 can add on day
	$option['GM_1_box5']         = 10  ;     // How many boxes +5 GM level 1 can add on day
	//////////////////////////              
	$option['GM_2_box1']         = 10 ;     // How many boxes +1 GM level 2 can add on day
	$option['GM_2_box2']         = 10 ;     // How many boxes +2 GM level 2 can add on day
	$option['GM_2_box3']         = 10 ;     // How many boxes +3 GM level 2 can add on day
	$option['GM_2_box4']         = 10  ;     // How many boxes +4 GM level 2 can add on day
	$option['GM_2_box5']         = 10  ;     // How many boxes +5 GM level 2 can add on day
	////////////////////////             
	
	//================================================================================================	
	//GM Account and Character Search
	$option ['user_pass_show']   = 0;       // Show User Login Password
